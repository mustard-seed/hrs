...
signal_rc = flipud(ifft(signal,[],1)); %range compressed signal ... flip needed due to sign convention of signal phase
JDE_TODO correct the path loss here (minor issue however)
imagesc(abs(signal_rc));figure(gcf);

signal_rc_ex = [signal_rc' (signal_rc(1,:))']'; %concatenate zero range
%row to the end to enable wrapped interpolation in the case when the range lies between last and first value
...
for ii = 1:length(x)
    ...
    for jj = 1:length(y)
        for kk = 1:length(z)
            ...
            test2 = GI(rTotal, t_segment)/fspl(rTotal, fc);
...
%--------------------------------------------------------------------------
function fspl_linear = fspl(range, fc)
    fspl_log = 20 * log10(range)+ 20 * log10(fc)-147.55; 
    fspl_linear = 10.^(-fspl_log/20);
end
...
for i_fc = 1:length(fc)
    ...
    FSPL_linear = fspl(rTotal, fc(i_fc));
    % Receive Power
    signal(i_fc, :) = FSPL_linear.*exp(complex(0,alpha + 2*pi*(rTotal)./lambda));
    %signal(i_fc, :) = exp(complex(0,alpha + 2*pi*(rTotal)./lambda));
end
end